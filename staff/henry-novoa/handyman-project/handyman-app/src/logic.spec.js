...
describe('logic', () => {
    ...
    describe('user', () => {
        describe('register', () => {
            ...
            it('should fail on empty name', () => {
                expect(() => logic.registerUser('', surname, username, password)).to.throw(Error, 'name is empty or blank')
            })

            it('should fail on blank name', () => {
                expect(() => logic.registerUser('   \t\n', surname, username, password)).to.throw(Error, 'name is empty or blank')
            })
            ...
            it('should fail on empty surname', () => {
                expect(() => logic.registerUser(name, '', username, password)).to.throw(Error, 'surname is empty or blank')
            })

            it('should fail on blank surname', () => {
                expect(() => logic.registerUser(name, '   \t\n', username, password)).to.throw(Error, 'surname is empty or blank')
            })
            ...
            it('should fail on empty username', () => {
                expect(() => logic.registerUser(name, surname, '', password)).to.throw(Error, 'username is empty or blank')
            })

            it('should fail on blank username', () => {
                expect(() => logic.registerUser(name, surname, '   \t\n', password)).to.throw(Error, 'username is empty or blank')
            })
            ...
            it('should fail on empty name', () => {
                expect(() => logic.registerUser(name, surname, username, '')).to.throw(Error, 'password is empty or blank')
            })
        ...
        describe('authenticate', () => {
            ...
            it('should authenticate on correct credentials', async () => {
                ...
                expect(_user.id).to.equal(logic._userId)
            })
            ...
            it('should fail on incorrect password', async () => {
                ...
                try {
                    await logic.login(username, notPassword)
                } catch (err) {
            ...
            it('should fail on incorrect password', async () => {
                ...
                try {
                    await logic.login(notUsername, password)
                } catch (err) {
                    expect(err).to.be.an.instanceOf(Error)
                    expect(err.message).to.equal('invalid username or password')
                }
            ...
            it('should fail on empty username', () => {
                expect(() => logic.login('', user.password)).to.throw(Error, 'username is empty or blank')
            })

            it('should fail on blank username', () => {
                expect(() => logic.login('   \t\n', user.password)).to.throw(Error, 'username is empty or blank')
            })
            ...
            it('should fail on empty password', () => {
                expect(() => logic.login(user.username, '')).to.throw(Error, 'password is empty or blank')
            })
        ...
        describe('retrieve', () => {
            ...
            it('should fail on empty id', () => {
                expect(() => logic.retrieveUser('')).to.throw(Error, 'id is empty or blank')
            })
    ...
    describe('Jobs', () => {
        ...
        describe('rate Job', () => {
            ...
            beforeEach(async () => {
                user = await new User({ name: 'John', surname: 'Doe', username: 'jd', password: '123' }).save()
                user2 = await new User({ name: 'Peter', surname: 'Griffin', username: 'pg', password: '123' }).save()
                ...
                job = await new Job({ title, photo, pictures, user: user.id, budget, contact, description, location, tags }).save()
            ...
            describe('with user requesting to do posted job', () => {
                ...
                beforeEach(async () => {
                    ...
                    expect(_job.requestedBy[0].toString()).to.equal(user.id)
                })
                ...
                describe('with user assigned', () => {
                    ...
                    beforeEach(async () => {
                        ...
                        const res = await logic.assignJob(user2.id, job.id)
                    ...
                    it('should correctly rate job', async () => {
                        ...
                        const res = await logic.rateJob(rating, ratingText,job.id)
                    ...
                    it('should fail on valid non-existant job Id', async () => {
                        ...
                        const ratingText = 'good job!!'
                        try {
                            const res = await logic.rateJob(rating, ratingText,notCorrectId)
                        } catch (err) {
                            expect(err).to.be.an.instanceOf(Error)
                    ...
                    it('should fail on valid non-string rating Text(number)', async () => {
                        const rating = 4
                        const notCorrectId = 4
                        const ratingText = 4
                        try {
                            const res = await logic.rateJob(job.id, rating, ratingText)
                        } catch (err) {
                            expect(err).to.be.an.instanceOf(TypeError)
                            expect(err.message).to.equal(`${notCorrectId} is not a string`)
                        }
                    ...
                    it('should fail on valid non-string rating text(boolean)', async () => {
                        const rating = 4
                        const notCorrectId = false
                        const ratingText = true
                        try {
                            const res = await logic.rateJob(rating, ratingText,job.id)
                        } catch (err) {
                            expect(err).to.be.an.instanceOf(TypeError)
                            expect(err.message).to.equal(`${ratingText} is not a string`)
                        }
                    })

                    it('should fail on valid non-string rating text(undefined)', async () => {
                        const rating = 4
                        const notCorrectId = undefined
                        const ratingText = undefined
                        try {
                            const res = await logic.rateJob(rating, ratingText,job.id)
                        } catch (err) {
                            expect(err).to.be.an.instanceOf(TypeError)
                    ...
                    it('should fail on valid non-string rating text(null)', async () => {
                        const rating = 4
                        const notCorrectId = null
                        const ratingText = null
                        try {
                            const res = await logic.rateJob(rating, ratingText,job.id)
                        } catch (err) {
                            expect(err).to.be.an.instanceOf(TypeError)
        ...
        describe('list', () => {
            ...
            it('(Jobs from one user) should succeed on correct data', async () => {
                const jobs = await logic.retrieveJobsFromUser(user.id)
            ...
            it('(Jobs from one user) should fail on non-existant(valid) userId', async () => {
                ...
                try {
                    ...
                    const jobs = await logic.retrieveJobsFromUser(notCorrectId)
                } catch (err) {
        ...
        describe('remove', () => {
            ...
            it('should succeed on correct data', async () => {
                const res = await logic.removeJob(job.id)
            ...
            it('should fail on valid non-existent job Id', async () => {
                const notCorrectId ='notCorrectId'
                 try{
                 const res = await logic.removeJob(notCorrectId)
                 }catch(err){
                     expect(err).to.be.instanceOf(Error)
                     expect(err.message).to.equal(`job with id ${notCorrectId} not found`)
        ...
        describe('request job', () => {
            ...
            false && describe('list requested by', () => {
                ...
                false && it('should correctly list users that request to do job', async () => {
        ...
        describe('assign Job', () => {
            ...
            describe('with user requesting to do posted job', () => {
                ...
                it('should fail on valid non-existent job Id', async () => {
                    ...
                    try{
                        const res = await logic.assignJob(user2.id, jobId)
                 ...
                 it('should fail on valid non-existent user id', async () => {
                    ...
                    try{
                        ...
                        const res = await logic.assignJob(userId, jobId)
